...
class BackupManager {
  ...
  // Create a new backup
  async createBackup(description: string = 'Manual backup'): Promise<{ success: boolean; backup?: BackupData; error?: string }> {
    try {
  ...
  async importBackup(file: File): Promise<{ success: boolean; backup?: BackupData; error?: string }> {
    return new Promise((resolve) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          const validation = this.validateBackup(backup);
          if (!validation.isValid) {
            resolve({ success: false, error: `Invalid backup file: ${validation.errors.join(', ')}` });
            return;
          }
  ...
  // Validate backup data
  private validateBackup(backup: BackupData): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    if (backup.data) {
      const requiredTables = ['companies', 'accounts', 'subAccounts', 'cashBookEntries', 'editHistory', 'users'];
    ...
    if (backup.metadata && backup.data) {
      const actualTotalRecords = Object.values(backup.data).reduce((sum, arr) => sum + arr.length, 0);
