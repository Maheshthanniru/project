...
const Input: React.FC<InputProps> = ({
  ...
  const filteredSuggestions = suggestions
    .map(String)
    .filter(s => inputValue && !String(inputValue).includes(s) && Math.abs(Number(s) - Number(inputValue)) < 10)
    .slice(0, 5);
  ...
  return (
    <div className={className} style={{ position: 'relative' }}>
      ...
      <input
        type={type}
        value={typeof value === 'number' && isNaN(value) ? '' : value === null || value === undefined ? '' : value}
